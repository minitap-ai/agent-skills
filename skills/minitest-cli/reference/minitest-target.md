...
- **App lifecycle:** the harness installs the build and launches the app with the agent bridge attached before the run starts. Launching is never Mini's job — a relaunch would replace the live bridge session, so Mini never launches the app itself. An app that never comes up is a Minitap harness failure to escalate to Minitap engineering — never a Mini limitation. *(Android, iOS)*
...
- **Microphone input:** play generated or bound audio into the device microphone so voice-driven flows are testable. *(Android, iOS)* **Android:** WAV and MP3 are accepted; use one-shot playback when the flow must return to silence automatically. **iOS:** WAV is the canonical test format; MP3, M4A and AAC are also accepted. Playback may run once or loop until stopped.
