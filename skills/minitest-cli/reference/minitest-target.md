...
- **System navigation:** press back to leave the current screen, and home to send the app to the background. *(Android, iOS)* **Android:** Both are real system key events. **iOS:** Home is the hardware button, but back is a left-edge swipe, so it only works where the app implements the swipe-back gesture — prefer the app's own back control when it has one.
