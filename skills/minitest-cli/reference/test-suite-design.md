...
**Credentials:** generate a passwordless `@qa.minitap.ai` persona for each profile and leave the password blank. Use a descriptive prefix derived from the role plus the app name, e.g. for an app called "FoodDash": `minitest-free@qa.minitap.ai`, `minitest-driver@qa.minitap.ai`. Leaving the password blank makes the persona OTP-first: at runtime the tester registers/signs in with that `@qa.minitap.ai` address and reads the confirmation or one-time code straight from its inbox — no real backend account needed. **Never invent a password or use a non-`@qa.minitap.ai` domain for a generated persona** — a passwordless non-`qa` address is rejected at creation, and a non-readable inbox defeats OTP. Only set a username + password when someone can actually provision that pair in the app's backend — the customer already owns a real account (any domain is then allowed), or has agreed to create the combo you hand them.

**The password branch exists only where the backend is ours to provision.** When the app under test belongs to someone else — a prospect's product, a public store download, an automated exploration nobody has onboarded — there is no owner to ask, so that branch is not available at all and every persona stays passwordless and OTP-first. Whatever account state such a persona needs is built at runtime from the signed-out app, never assumed to be sitting behind a password you made up.

**Apps that sign in by phone** take a `phone_number` (E.164, e.g. `+14155551234`) instead of, or alongside, the address — plus a `static_otp_code` when the customer's backend accepts a fixed code for that number. Both must be pre-provisioned by the customer: ask them for a whitelisted number and its fixed code, because there is no phone inbox to read at runtime the way there is for `@qa.minitap.ai`. A persona that carries a phone number or a static OTP code has its own way in, so it keeps whatever address you give it — a real customer address is accepted, and no `@qa.minitap.ai` inbox is invented for it. **For a persona that needs a specific account state (e.g. premium/pro), generate a `<something>@qa.minitap.ai` persona *with* an explicit password and ask the user to add that email + password combo to their backend** so it is linked to a user in that state — the `qa` address keeps the inbox readable for OTP while the password lets them pre-provision the account. That route is open only when there is a user to ask and a backend they control; with no such owner, the persona stays passwordless and reaches the state through the app itself. The `about` field must explain who this persona is and what state they need to be in (e.g. "Pro subscriber with an active monthly plan. Has completed onboarding and has at least one saved item.").
...
**A persona's way in is a claim you must verify, never one you may assume.** Whichever route you record — email + password, an OTP to a readable inbox, a phone number with a static code, a leased Google account — record the one that has been demonstrated **from a signed-out state**. Registering once and staying signed in for the rest of an exploration proves the account exists; it says nothing about whether that credential can sign in again on a clean device, which is precisely the state every run starts from. So before emitting the access-gate story the rest of the suite hangs off, sign out and sign back in with the stored credential. If that round trip cannot be demonstrated, record the route that actually worked — usually OTP — instead of the one you wish were true.

Getting this wrong costs the suite, not a story. The access gate is normally the root of the dependency graph, so an unverified gate does not fail alone: it fails critically and the cascade skips everything beneath it. An automated prospect run bound all twenty of its stories to a generated persona whose invented password had never signed anything in; the app answered "This email isn't linked to an account", the sign-in story failed critical, the other nineteen cascade-skipped, and the run produced zero verdicts — no coverage, no bugs found, nothing to read.
